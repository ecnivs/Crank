...
class VideoEditor:
    ...
    def _get_duration(self, file_path):
        if not os.path.exists(file_path):
            raise FileNotFoundError(f"[{self.__class__.__name__}] File not found: {file_path}")
        cmd = [
            "ffprobe",
            "-v", "error",
        ...
        try:
            ...
            if 'format' not in info or 'duration' not in info['format']:
                raise RuntimeError(f"[{self.__class__.__name__}] Could not extract duration from {file_path}")

            return float(info['format']['duration'])
        except subprocess.CalledProcessError as e:
            raise RuntimeError(f"[{self.__class__.__name__}] FFprobe failed: {e.output.decode() if hasattr(e, 'output') else str(e)}")
    ...
    def process_video(self, ass_path, audio_path, media_path):
        ...
        for file_path, file_desc in [
            ...
            if not os.path.exists(file_path):
                raise FileNotFoundError(f"[{self.__class__.__name__}] {file_desc} not found: {file_path}")
